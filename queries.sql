...
VALUES
	('hero34@mail.ru', 'Ярослав', '1111', '888777666'),
  ('asis174@mail.ru', 'Слава', '2222', '555444333');
...
FROM lots
JOIN categories USING(categories.id);
...
FROM lots JOIN categories  USING(categories.id)
WHERE lots.id=4;
...
FROM bets
JOIN lots USING(lots.id)
JOIN users USING(users.id)
WHERE lots.id=4
